...
class EEG_Processing_User {
  ...
  EEG_Processing_User(int NCHAN, float sample_rate_Hz) {
    ...
    // To send the values to Unity Game
    oscP5 = new OscP5(this,12000);
    myRemoteLocation = new NetAddress("10.240.192.190",9002);
  }
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    for (int Ichan=0;Ichan < nchan; Ichan++) {
      ...
      for (int Isamp=indexOfNewData; Isamp < data_forDisplay_uV[Ichan].length; Isamp++) {
        ...
        if(Ichan==1)
         {  //println("Here EEG_value_uV : "+EEG_value_uV);
           if(EEG_value_uV>50.0){ // A blinking happened here
             println("Blinking");
                OscMessage oscMess = new OscMessage("/blinking");
                oscMess.add(1);//1 Like True
                oscP5.send(oscMess, myRemoteLocation);
           }else{
                println("nothing");
                OscMessage oscMess = new OscMessage("/blinking");
                oscMess.add(0); //0 Like false
                oscP5.send(oscMess, myRemoteLocation);
